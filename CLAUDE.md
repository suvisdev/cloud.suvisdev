...
| 3 | `vault/DevOps/Backend/ENTITY_RULE.md` | PK `id` int 자동증감 |
| 4 | `vault/DevOps/Backend/MOVA_ERD.md` | Mova 테이블 관계 (해당 시) |
...
| 앱 | 역할 | 표준 참조 |
|----|------|-----------|
| `titanic/` | CSV 업로드(James)·조회(Walter) — **헥사고날 기준선** | `crew_james_director_interactor.py`, `dependencies/crew_james_director_provider.py` |
...
| Output Port | `titanic/app/ports/output/crew_james_director_repository.py` |
| PgRepository | `titanic/adapter/outbound/pg/crew_james_director_pg_repository.py` |
| DI | `titanic/dependencies/crew_james_director_provider.py` |
| DTO | `titanic/app/dtos/crew_james_director_dto.py` (`PersonCommand`, `JamesResponse`) |
...
| 코딩 규칙 인덱스 | `vault/README.md` |
| 엔티티 PK | `vault/DevOps/Backend/ENTITY_RULE.md` |
| Mova ERD | `vault/DevOps/Backend/MOVA_ERD.md` |
...
## N. Harness Engineering: Wiki + LLM PKS (Personal Knowledge System)

### N.1 Purpose

This section governs the implementation of Karpathy-style harness engineering
for a **Wiki + LLM PKS (Personal Knowledge System)** inside this repository.
All code generated for the PKS must adhere to:

- **SOLID:** SRP, ISP, DIP are always applied. OCP and LSP are added only when explicitly requested — no speculative abstractions.
- **Hexagonal Architecture (Ports & Adapters):** domain core is isolated behind Input Ports (Use Case ABCs) and Output Ports (Repository ABCs). All external concerns (HTTP, DB, LLM) live in adapters.
- **Clean Architecture:** dependency arrow points strictly **inward** (outer layers depend on inner layers, never the reverse).
- **DDD Tactical Patterns:** Entities, Value Objects, Repositories (as Output Ports), Domain Services, and Application Services (Interactors) are the building blocks.

Karpathy's four principles (explicit assumptions, simplicity first, surgical changes, goal-driven execution) remain in force for all PKS work — see sections 1–4 above.

---

### N.2 Import Path Convention

| Scope | Rule | Example |
|-------|------|---------|
| **App modules** | Omit `suvisdev.apps.` prefix — treat `suvisdev/apps/` as the import root | `from mova.app.ports.input.movies_use_case import MoviesUseCase` |
| **Core modules** | `core.*` (main.py가 `suvisdev/`를 path에 추가) | `from core.matrix.grid_oracle_database_manager import get_db` |
| **Unknown paths** | Consult `AGENTS.md` first — never guess | — |

No other path conventions are permitted. Agents must not infer paths from naming patterns; always verify against `AGENTS.md` or the directory tree before writing an import.

---

### N.3 Layer Map — Hexagonal + Clean + DDD for PKS

| Layer | DDD Concept | Location pattern |
|-------|-------------|-----------------|
| **Domain Entity / Value Object** | Entity, VO | `{app}/app/dtos/*_model.py` or `{app}/app/domain/` |
| **Output Port (Repository ABC)** | Repository interface | `{app}/app/ports/output/*_repository.py` |
| **Input Port (Use Case ABC)** | Application Service interface | `{app}/app/ports/input/*_use_case.py` |
| **Application Service (Interactor)** | Application Service impl | `{app}/app/use_cases/*_interactor.py` |
| **Inbound Adapter (Router)** | — | `{app}/adapter/inbound/api/v1/*_router.py` |
| **Outbound Adapter (PgRepository)** | Repository impl | `{app}/adapter/outbound/pg/*_pg_repository.py` |
| **LLM Adapter** | — | `{app}/adapter/outbound/llm/*_llm_adapter.py` |
| **DI (Composition Root)** | — | `{app}/dependencies/*_provider.py` |
...
| ❌ Forbidden | ✅ Instead |
|-------------|-----------|
| `from suvisdev.apps.mova...` (long prefix) | `from mova...` (short app-root path) |
| `from suvisdev.core...` long prefix in runtime code | `from core.matrix.grid_oracle_database_manager import ...` |
| Domain logic in Router or PgRepository | Move to Interactor or Domain Service |
| Passing raw `dict` to Repository instead of a Command object | `Command.from_schema(schema)` → pass Command |
| Instantiating `XxxPgRepository()` inside an Interactor | Constructor injection via `dependencies/*.py` |
| Adding OCP/LSP abstractions not requested by the user | YAGNI — only SRP, ISP, DIP by default |
| Placing LLM API calls directly in the Router | Wrap in an outbound LLM Adapter; call via Output Port |

---

### N.5 Checklist — Adding a New PKS Feature

1. [ ] `schemas/*_schema.py` — Pydantic Request / Response schemas
2. [ ] `app/dtos/` — `*Command.from_schema`, `*Dto.from_orm`, `Dto.to_schema`
3. [ ] `app/ports/input/*_use_case.py` — ABC (Schema in, Dto out)
4. [ ] `app/ports/output/*_repository.py` — ABC (Command in)
5. [ ] `app/use_cases/*_interactor.py` — Schema → Command → Port → Dto
6. [ ] `adapter/outbound/pg/*_pg_repository.py` — Command handling, `RepositoryError`
7. [ ] `adapter/outbound/llm/*_llm_adapter.py` — LLM call wrapped as outbound adapter (if needed)
8. [ ] `dependencies/*.py` — `get_*_use_case` wiring
9. [ ] `adapter/inbound/api/v1/*_router.py` — `Depends`, `invoke`, `Dto.to_schema()`
10. [ ] Include router in `main.py` or the app's top-level router
11. [ ] Run `python -c "import suvisdev.main"` — must import cleanly
12. [ ] Run `python scripts/verify_db_tables.py` if new ORM tables were added
